...
namespace Game.Unity.Tests
{
    ...
    {
        [Test]
        public void RetryFromMemoryPopupHasCloseButtonAndAutoCloses()
        {
            var payload = MemoryPathPopups.RetryFromMemory();
            Assert.That(payload.Chrome, Is.EqualTo(MemoryPathPopupChrome.Fail));
            Assert.That(payload.Title, Is.EqualTo("Try again from memory"));
            Assert.That(payload.Message, Does.Contain("same path"));
            Assert.That(payload.Buttons, Is.Empty);
            Assert.That(payload.ShowCloseButton, Is.True);
            Assert.That(payload.CloseOnBackdrop, Is.True);
            Assert.That(payload.TitleSize, Is.GreaterThanOrEqualTo(28f));
            Assert.That(payload.MessageSize, Is.GreaterThanOrEqualTo(22f));
            Assert.That(payload.PanelWidth, Is.GreaterThan(MemoryPathPopup.S(350)));
            ...
            try
            {
                ...
                Assert.That(popup.transform.Find("Panel/Stack").gameObject.activeSelf, Is.False);
                var close = popup.transform.Find("Panel/Close");
                Assert.That(close, Is.Not.Null);
                Assert.That(close.gameObject.activeSelf, Is.True);
                Assert.That(close.GetComponent<Button>(), Is.Not.Null);
                Assert.That(
                    popup.transform.Find("Panel/Body/Title").GetComponent<TextMeshProUGUI>().text,
                    Is.EqualTo("Try again from memory"));
            }
            finally
            {
        ...
        public void CompletePayloadUsesGoldTitleStarsAndStackedButtons()
        {
            ...
            Assert.That(payload.Buttons[1].Label, Is.EqualTo("Replay"));
            ...
            var last = MemoryPathPopups.Complete(1, false, null, null, null);
            Assert.That(last.Buttons[0].Label, Is.EqualTo("Replay"));
        ...
        public void FailAndPausePayloadsMatchFigma()
        {
            ...
            Assert.That(fail.Buttons[0].Label, Is.EqualTo("Try Again"));
            ...
            Assert.That(pause.Glyph, Is.EqualTo(MemoryPathGlyph.Pause));
            Assert.That(pause.Panel, Is.EqualTo(MemoryPathPalette.PopupPause));
            Assert.That(pause.Title, Is.EqualTo("Take a breather!"));
        ...
        public void CompletePopupStacksButtonsAndShowsScore()
        {
            ...
            try
            {
                ...
                Assert.That(panel.Find("Stack/Btn1/Label").GetComponent<TextMeshProUGUI>().text, Is.EqualTo("Replay"));
        ...
        public void FailPopupShowsHeartGlyphAndPauseShowsBars()
        {
            ...
            try
            {
                ...
                Assert.That(popup.transform.Find("Panel/Stack/Btn0/Label").GetComponent<TextMeshProUGUI>().text, Is.EqualTo("Try Again"));
                ...
                Assert.That(glyph.gameObject.activeSelf, Is.True);
                Assert.That(glyph.Find("Inner/Heart").gameObject.activeSelf, Is.False);
                Assert.That(glyph.Find("Inner/Pause").gameObject.activeSelf, Is.True);
